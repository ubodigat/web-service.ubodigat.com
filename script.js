...
function setTheme(theme) {
  ...
  try { localStorage.setItem(themeKey, theme); } catch { /* blocked */ }
}

function initTheme() {
  let saved = "dark";
  try { saved = localStorage.getItem(themeKey) || "dark"; } catch { saved = "dark"; }
  setTheme(saved);
...
function initParticles() {
  ...
  let width = 0, height = 0, raf = 0, particles = [];
...
function initFilemanager() {
  ...
  const FS = {
    "/var/www/html": [
      { name: "config",       type: "folder",   size: "994 B",      date: "13.06.2026 00:30", status: "locked",  canOpen: true  },
      { name: "filemanager",  type: "folder",   size: "4.16 MB",    date: "13.06.2026 22:20", status: "locked",  canOpen: false },
      { name: "install",      type: "folder",   size: "27.44 KB",   date: "13.06.2026 00:30", status: "locked",  canOpen: false },
      { name: "projekt",      type: "folder",   size: "1.48 GB",    date: "13.06.2026 15:24", status: "public",  canOpen: true  },
      { name: "index.php",    type: "php",      size: "4.00 KB",    date: "13.06.2026 15:28", status: "public",  editable: true },
      { name: "preview.webp", type: "image",    size: "312.00 KB",  date: "13.06.2026 14:10", status: "public"               },
    ],
    "/var/www/html/config": [
      { name: "db.env",       type: "document", size: "128 B",      date: "13.06.2026 00:30", status: "locked"  },
      { name: ".htaccess",    type: "web",      size: "384 B",      date: "13.06.2026 00:30", status: "locked"  },
    ],
    "/var/www/html/projekt": [
      { name: "bilder",       type: "folder",   size: "892.00 MB",  date: "12.06.2026 11:20", status: "public",  canOpen: true  },
      { name: "css",          type: "folder",   size: "124.00 KB",  date: "11.06.2026 09:14", status: "public",  canOpen: true  },
      { name: "js",           type: "folder",   size: "88.00 KB",   date: "11.06.2026 09:14", status: "public",  canOpen: true  },
      { name: "index.php",    type: "php",      size: "8.22 KB",    date: "12.06.2026 17:34", status: "public",  editable: true },
      { name: "README.md",    type: "document", size: "2.10 KB",    date: "10.06.2026 08:00", status: "public",  editable: true },
      { name: "backup.zip",   type: "archive",  size: "156.40 MB",  date: "13.06.2026 01:00", status: "locked"               },
    ],
    "/var/www/html/projekt/bilder": [
      { name: "hero.webp",    type: "image",    size: "642.00 KB",  date: "12.06.2026 10:12", status: "public" },
      { name: "logo.png",     type: "image",    size: "48.00 KB",   date: "10.06.2026 08:00", status: "public" },
      { name: "demo.mp4",     type: "video",    size: "124.80 MB",  date: "13.06.2026 12:44", status: "public" },
    ],
    "/var/www/html/projekt/css": [
      { name: "style.css",    type: "web",      size: "28.44 KB",   date: "11.06.2026 09:14", status: "public",  editable: true },
    ],
    "/var/www/html/projekt/js": [
      { name: "main.js",      type: "web",      size: "18.22 KB",   date: "11.06.2026 09:14", status: "public",  editable: true },
    ],
  };
  ...
  function navigateTo(path) {
    if (!FS[path]) { notify("Ordner nicht geöffnet (Demo-Einschränkung)."); return; }
    currentPath = path;
  ...
  /* refresh */
  if (refresh) refresh.addEventListener("click", () => { renderTable(); notify("Dateiliste aktualisiert."); });

  /* create file */
  if (createFile && newFileIn) {
    createFile.addEventListener("click", () => {
      const fname = newFileIn.value.trim();
      if (!fname) { notify("Bitte Dateinamen eingeben."); return; }
      const ext = fname.split(".").pop().toLowerCase();
      const typeMap = { php: "php", html: "web", css: "web", js: "web", md: "document", txt: "document", webp: "image", png: "image", jpg: "image", mp4: "video", zip: "archive" };
      const ftype = typeMap[ext] || "document";
      FS[currentPath].push({ name: fname, type: ftype, size: "0 B", date: new Date().toLocaleString("de-DE").replace(",",""), status: "public", editable: true });
      newFileIn.value = "";
      renderTable();
  ...
  if (createFolder && newFolderIn) {
    createFolder.addEventListener("click", () => {
      ...
      if (!fname) { notify("Bitte Ordnernamen eingeben."); return; }
      FS[currentPath].push({ name: fname, type: "folder", size: "0 B", date: new Date().toLocaleString("de-DE").replace(",",""), status: "public", canOpen: true });
  ...
  const LANG_MAP = { php: "PHP", css: "CSS", js: "JavaScript", md: "Markdown", env: "ENV", htaccess: "Apache Config" };
  ...
  window._fmDelete  = name => {
    ...
    if (idx !== -1) { FS[currentPath].splice(idx, 1); renderTable(); }
  ...
  if (settingsBtn && settingsPopup) {
    ...
    if (saveSettings) saveSettings.addEventListener("click", () => { closeSettings(); notify("Einstellungen gespeichert (Demo)."); });
...
function initAdmin() {
  ...
  btn.addEventListener("click", () => {
    ...
    window.setTimeout(() => {
      ...
      if (msg) msg.textContent = "Aktuelle Version: 1.6.9  →  Neue Version: 1.7.0";
      if (badge) badge.classList.remove("hidden");
      if (doUpdate) doUpdate.disabled = false;
      btn.disabled = false;
      notify("Update verfügbar: v1.7.0");
    }, 1400);
  });
  ...
  if (doUpdate) {
    doUpdate.addEventListener("click", () => {
      ...
      window.setTimeout(() => {
        if (badge) badge.classList.add("hidden");
        if (msg)   msg.textContent = "Update auf v1.7.0 erfolgreich (Demo).";
        const ver = $("#currentVersion");
        if (ver)   ver.textContent = "1.7.0";
        status.textContent = "Version: 1.7.0 aktuell";
        btn.disabled = false;
        notify("Update auf v1.7.0 eingespielt (Demo).");
      }, 1600);
    });
  }
}
...
function initPermissions() {
  ...
  function attachRuleButtons(ruleEl) {
    ruleEl.querySelector(".mini.red")?.addEventListener("click", () => {
      ruleEl.remove(); notify("Regel gelöscht.");
    });
